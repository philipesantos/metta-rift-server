...
def build_world():
    character_player = Character(key="player", name="John")

    location_glade = Location(key="glade", desc="You are in the Glade.")

    location_cave = Location(key="cave", desc="You are in the cave.")

    location_boat = Location(key="boat", desc="You are in the boat.")

    location_plane = Location(key="plane", desc="You are in the plane.")

    location_cabin = Location(key="cabin", desc="You are in the cabin.")

    location_camping_site = Location(
        key="camping_site", desc="You are in the camping site."
    )

    location_monolith = Location(key="monolith", desc="You are in the monolith.")

    location_path_1 = Location(key="path_1", desc="You are in the path 1.")

    location_path_2 = Location(key="path_2", desc="You are in the path 2.")

    location_path_3 = Location(key="path_3", desc="You are in the path 3.")

    location_path_4 = Location(key="path_4", desc="You are in the path 4.")

    location_path_5 = Location(key="path_5", desc="You are in the path 5.")

    location_path_6 = Location(key="path_6", desc="You are in the path 6.")

    location_path_7 = Location(key="path_7", desc="You are in the path 7.")

    location_path_8 = Location(key="path_8", desc="You are in the path 8.")

    location_path_9 = Location(key="path_9", desc="You are in the path 9.")
    ...
    world.add_function(
        Trigger(
            MoveEvent("$from", "$to"),
            [OnMoveUpdateAt(character_player), OnMoveUpdateTick()],
        )
    )
