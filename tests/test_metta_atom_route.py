...
class TestMettaAtomRoute(unittest.TestCase):
    def test_to_metta_usage(self):
        ...
        self.assertEqual(
            route_metta_usage, f"(Route {location_from} {direction} {location_to})"
        )
    ...
    def test_to_metta_definition(self):
        ...
        route_metta_definition = Route(
            location_from, direction, location_to
        ).to_metta_definition()
        ...
        route_metta_usage_direction = Route.to_metta_usage(
            location_from, "$direction", location_to
        )
        result_direction = metta.run(
            f"!(match &self {route_metta_usage_direction} $direction)"
        )
        ...
        route_metta_usage_no_match = Route.to_metta_usage(
            "beach", direction, location_to
        )
