...
class World:
    ...
    def add_route(
        self, location_from: Location, direction: Direction, locations_to: Location
    ):
        self.routes.append(Route(location_from.key, direction.value, locations_to.key))
        self.routes.append(
            Route(locations_to.key, direction.opposite.value, location_from.key)
        )
    ...
    def to_metta_definition(self) -> str:
        line_break = "\n\n"
        functions_metta = line_break.join(
            [function.to_metta_definition() for function in self.functions]
        )
        characters_metta = line_break.join(
            [character.to_metta_definition() for character in self.characters]
        )
        locations_metta = line_break.join(
            [location.to_metta_definition() for location in self.locations]
        )
        routes_metta = line_break.join(
            [route.to_metta_definition() for route in self.routes]
        )
        ats_metta = line_break.join([at.to_metta_definition() for at in self.ats])
        current_ats_metta = line_break.join(
            [
                current_at.to_metta_definition()
                for current_at in self.current_ats.values()
            ]
        )
